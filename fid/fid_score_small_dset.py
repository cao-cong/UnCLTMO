...
def get_activations_for_small_dataset(files, model, batch_size=50, dims=2048,
                    cuda=False, verbose=False, is_real=False):
...
def calculate_activation_statistics(files, model, batch_size=50,
                                    dims=2048, cuda=False, verbose=False, is_real=False):
    ...
    -- sigma : The covariance matrix of the activations of the pool_3 layer of
               the inception model.
    """
    act = get_activations_for_small_dataset(files, model, batch_size, dims, cuda, verbose, is_real)
    mu = np.mean(act, axis=0)
    sigma = np.cov(act, rowvar=False)
    return mu, sigma


def _compute_statistics_of_path(path, model, batch_size, dims, cuda, is_real=False):
...
def calculate_fid_given_paths(paths, batch_size, cuda, dims):
    ...
    #print("real model")
    #summary(model, (3, 299, 299), device="cuda")
    # if cuda:
    if torch.cuda.is_available():
        model.cuda()
...
if __name__ == '__main__':
    ...
    if os.path.exists(args.path_to_save):
        ...
        data[args.model_name] = fid_value
