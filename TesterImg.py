...
class Tester:
    ...
    def save_images_for_model(self, netG, out_dir, epoch, epoch_iter):
        ...
        f_factor_path = "activate_trained_model/lambda_data/input_images_lambdas_HDRSurveydataset.npy"
        input_images_path = "../../data/tone_mapping/HDR_Survey_dataset"
        ...
        for img_name in names:
            ...
            tmqi_scene = self.eval_on_image(netG, im_path, self.device, im_name, f_factor_path, self.args.final_shape_addition, epoch, epoch_iter)
            tmqi_final += tmqi_scene

        tmqi_final = tmqi_final/len(names)

        out_dir = os.path.join(out_dir, "model_results", "epoch"+str(epoch)+"_iter"+str(epoch_iter)+"_tmqi"+str(tmqi_final))
        ...
        with torch.no_grad():
            for im_and_q in self.test_original_hdr_images:
                ...
                fake = self.test_big_size_image(im_log_normalize_tensor, model=netG, apply_crop=self.to_crop, diffY=im_and_q['diffY'], diffX=im_and_q['diffX'], patch_h = 256, patch_w = 256, patch_h_overlap = 64, patch_w_overlap = 64)
    ...
    def eval_on_image(self, G_net, im_path, device, im_name, f_factor_path, final_shape_addition, epoch, epoch_iter):
        #gray_im_logs = []
        #rgb_imgs = []
        #rgb_imgs_original = []
        #gray_ims_log = []
        ...
        fake = self.test_big_size_image(input_data=gray_im_log.unsqueeze(0), model=G_net, apply_crop=False, diffY=diffY, diffX=diffX, patch_h = 256, patch_w = 256, patch_h_overlap = 64, patch_w_overlap = 64)
        ...
        #gauss = gaussian(11, 20)
        #window = np.outer(gauss, gauss)
        score, s_score, n_score, _, _ = tmqi(rgb_img_original, ldr_result.astype(np.float32))
        #score, s_score, n_score, _, _ = tmqi(rgb_img_original, ldr_result.astype(np.float32), window)
        return score
